 ...
 */
export const fetchWithAuthentication = (csrfUrl, fetchUrl, fetchOptions) => {
    if (!fetchOptions.headers.get('X-CSRF-Token')) {
      return fetchWithAuthentication(csrfUrl)
        .then(response => response.text())
        .then((csrfToken) => {
          fetchOptions.headers.append('X-CSRF-Token', csrfToken);
          return fetchWithAuthentication(fetchUrl, fetchOptions);
        });
    }
    else {
      return fetchWithAuthentication(fetchUrl, fetchOptions);
